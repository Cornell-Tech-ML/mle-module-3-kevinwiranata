...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    # index = array index (i.e. (0, 1, 2) in a 3x3 tensor)
    # position = position storage by strides
    # broadcast_index = find the correct index in the input tensor that maps to the index in the new output tensor
    def _map(
        out: Storage,
        out_shape: Shape,
        out_strides: Strides,
